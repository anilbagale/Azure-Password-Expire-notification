...
# As per Azure password policy change the days in line no 39 to get password expiration date. EX: 45, 60, 90
...
# Format the date and add 90 days
newDate=$(date -j -u -v+90d -f "%Y-%m-%dT%H:%M:%SZ" "$lastPasswordChangeDateTime" "+%Y-%m-%d %H:%M:%S")

# Output the new date to the extension attribute
echo "<result>$newDate</result>"